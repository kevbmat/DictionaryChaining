...
// determines which index in the hashTable the
// phone number hashes to.
// pre: theKey has been assigned to a 7-digit phone number
// post: the sum of the 7-digit number modded by the tablesize
// is returned.
...
// helps to print out the stack of Item objects
// pre: aStack has been assigned to a stack of Item objects
// post: if there are Item objects left in the stack
//      then output contains each Item printed out on a line
...
// helps to search for a contact with a particular phone number
// pre targetPhone has been assigned a 7-digit phone number
// and aStack has been assigned to a stack of Item objects.
...
{
    Item nextItem;
    while (not aStack.isEmpty() && not isFound) {
        aStack.getTop(nextItem);
        if (nextItem == targetPhone)
        {
            isFound = true;
            theItem = nextItem;
        }
...
// pre: targetPhone is assigned and aStack has been assigned to a stack of Item objects.
// post: if targetPhone is found in Dictionary object, isDone is true
//           and the associated Item object (phone and name) has been
//           removed from the Dictionary as well as from aStack
//       else isDone is false
void removeHelper (const Key& targetPhone, bool& isDone, Stack& aStack)
{
    Item nextItem;
    Stack copyStackA;
    copyStackA = aStack;
    Stack copyStackB;
    bool isAdded;
    while (not copyStackA.isEmpty())
    {
        copyStackA.getTop(nextItem);
        if (not (nextItem == targetPhone))
        {
            copyStackB.push(nextItem, isAdded);
        }
        else
        {
            isDone = true;
    ...
    while (not copyStackB.isEmpty())
    {
        copyStackB.getTop(nextItem);
        copyStackA.push(nextItem, isAdded);
        copyStackB.pop();
    }
    aStack = copyStackA;
}
...
istream& operator>> (istream& input, Dictionary& rightHandSideDictionary)
{
    ...
    input >> numberItems;
    input.get();
    if (rightHandSideDictionary.dictionaryPtr->numberOfItems < TABLESIZE)
    {
        isFull = false;
        for(int i = 0; i < numberItems; i++)
        {
            ...
            if (!isFull)
            {
                rightHandSideDictionary.insert(nextItem, isFull);
            }
        }
    }
    return input;
...
void Dictionary::search (const Key& targetPhone, Item& theItem, bool& isFound)
{
    ...
    if (not dictionaryPtr -> hashPtr[hashAddress].isEmpty())
    {
...
void Dictionary::remove (const Key& targetPhone, bool& isDone)
{
    ...
    if (not dictionaryPtr -> hashPtr[hashAddress].isEmpty())
    {
        removeHelper(targetPhone, isDone, dictionaryPtr -> hashPtr[hashAddress]);
    }
